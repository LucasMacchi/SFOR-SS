...
import { viajeDetalleSQL, viajeRemitoSQL, viajeUniqSQL } from "./SQLreturner";
import authJwt from "@/utils/authJwt";
import decodeJWT from "@/utils/decodeJWT";

export default async function (id:number): Promise<IViajeRQ[]> {
    const conn = clientReturner()
    try {
        const user = await decodeJWT()
        if(await authJwt(3) && user) {
            await conn.connect()
            const viajes:IViajeRQ[] = (await conn.query(viajeUniqSQL(user.userId))).rows
