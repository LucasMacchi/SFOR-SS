import { IAddEnvio, IAddRemito, IEnvioDetalles, IInsumo, IPlan, IViajeRemito, IViajeRQ } from "./interfaces";



export default function (remitosP:IViajeRemito[],planes: IPlan[],insumos:IInsumo[],reparto:number) {
                        ...
                        if(pd.ins_id === ins.ins_id) {
                            if(plan.celiacos || plan.diabetes || plan.mixto) {
                                ...
                                enviosDetalles.push({
                                    ...
                                    ins_id: ins.ins_id,
                                    insumo: ins.des,
                                    unidades: Math.ceil(racIns / ins.racunidad) !== 0 ? Math.ceil(racIns / ins.racunidad) : 1,
                                    raciones: Math.ceil(racIns / ins.racunidad) * ins.racunidad !== 0  ? Math.ceil(racIns / ins.racunidad) * ins.racunidad : 1 * ins.racunidad
                                })
                            }
                        }
                    })
