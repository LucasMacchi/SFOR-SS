import { IDesgloseParsed, IEnvioDetallesParsed, IInsumo, IPlan, IViajeRemitoRQ, IViajeRQ } from "./interfaces";

export default function (insumos: IInsumo[],planes: IPlan[], viajes: IViajeRQ[]) {

    const detalles: IDesgloseParsed[] = []

    viajes.forEach((viaje) => {
        viaje.remitos.forEach((r) => {
            planes.forEach(p => {
                        ...
                        p.detalles.forEach(pd => {
                            insumos.forEach(ins => {
                                if(ins.ins_id === pd.ins_id) {
                                    const unidadesN = Math.floor((rd.raciones * pd.dias) / ins.racunidad) ? Math.floor((rd.raciones * pd.dias) / ins.racunidad) : 1
