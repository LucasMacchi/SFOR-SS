...
const insumosDisplayer = (remitos: IRemitoInFactura[],count: number,last:boolean,totalAmount: number,totalRac: number,totalRts: number,valor:number) => {
    const elements = []
    ...
    for (let i = 0; i < amount; i++) {
        elements.push(
            <View style={stylePedido.tableRow}>
                ...
                <View style={stylePedido.tableColcod2}>
                    <Text style={stylePedido.tableCell2}>{remitos[i] ? Intl.NumberFormat("es-AR",{style: "currency", currency: "ARS"}).format(remitos[i].raciones * valor) : " "}</Text>
                </View>
            </View>
        )
...
const PDFfactura = ({factura,valor}:{factura: IFacturaAgrupado,valor:number}) => {
    ...
    const amountTotal = factura.raciones * valor
    const racionesTotal = factura.raciones
    return (
        <Document>
            <Page size={'A4'} style={stylePedido.page}>
                ...
                <View style={stylePedido.viewdataHeader}>
                    <View >
                        ...
                        <Text style={stylePedido.subtitle}>Fecha: {factura.fecha_factura.toISOString().split("T")[0]}</Text>
                    </View>
                    <View >
            ...
            {pages.map((d,i) => {
                ...
                if(i !== 0) {
                    return (
                        <Page size={'A4'} style={stylePedido.page}>
                            {insumosDisplayer(d,paginasCount,isLast,amountTotal,racionesTotal,remitos.length,valor)}
                        </Page>
                    )
                }
            })}
