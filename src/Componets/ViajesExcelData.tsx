"use client"
import {IViajeExcelComplete, IViajeExcelCompleteInsumos, IViajeExcelRQ } from "@/utils/interfaces";
import Image from "next/image"
import * as XLSX from 'xlsx';


export default function ViajesExcelData ({getViajesDataProcesed}:{getViajesDataProcesed: () => Promise<IViajeExcelCompleteInsumos[]>}) {

    const color = "#32CD32"

    const createExcel = async () => {
        const data = await getViajesDataProcesed()
        if(data.length > 0) {
